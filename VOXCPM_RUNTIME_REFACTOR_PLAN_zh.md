...
- `2026-04-20`: `AudioVAE` chunk decode 的默认 chunk size 已从固定 `64` 帧改为自动填满安全窗口；实测普通 decoder 在 `1280` latent patches 也会触发 CUDA transpose copy 上限，因此 GPU 单窗口统一收紧到 `1024` patches，VoxCPM1.5 默认可从 `64` 帧 chunk 扩到约 `192` 帧 chunk，减少重复 overlap decode 且避开 `ggml-cuda cpy` 断言。
- `2026-04-20`: VoxCPM2 / conditioned decoder 的默认 chunk history 已从 `192` 调整为 `128` 帧；在同一 `1024` patch 安全窗口内，默认 chunk 从 `64` 帧扩大到 `128` 帧，实测长文本 `AudioVAE decode` 从约 `13.5s` 降到约 `9.8s`。如需保守质量可通过 `VOXCPM_AUDIO_DECODE_HISTORY_FRAMES=192` 恢复旧 overlap。
